  ...
  return (
    <Field label={label}>
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {/* `relative` NO es decorativo: el input de archivo va `sr-only`, que es
              `position: absolute`, y a un absoluto solo lo recorta un ancestro que sea
              su bloque contenedor. Con el label `static` se anclaba al DOCUMENTO,
              escapaba del `overflow-y: auto` del panel y estiraba el `<html>` —medido:
              1524px de alto contra 768 de ventana, 756px de vacío debajo del editor—.
              Anclado a su propio label viaja con él y el panel vuelve a recortarlo. */}
          <label className="relative inline-flex cursor-pointer rounded-lg border border-outline-variant/30 px-3 py-2 text-xs font-bold text-on-surface">
            <input
              type="file"
...
function GalleryEditor({ config, onChange }: PanelProps) { const uploadImage = useBusinessSiteStore((state) => state.uploadImage); const uploading = useBusinessSiteStore((state) => state.uploading); return <div className="space-y-3"><div className="grid grid-cols-3 gap-2">{config.gallery.images.map((image) => <div key={image.id} className="rounded-lg border border-outline-variant/30 p-2"><div className="aspect-square rounded bg-surface-container bg-cover bg-center" style={{ backgroundImage: `url(${image.url})` }} /><input value={image.alt} onChange={(event) => onChange((current) => ({ ...current, gallery: { ...current.gallery, images: current.gallery.images.map((item) => item.id === image.id ? { ...item, alt: event.target.value } : item) } }))} className="mt-2 w-full bg-transparent text-[11px] text-on-surface" placeholder="Descripción" /><button type="button" onClick={() => onChange((current) => ({ ...current, gallery: { ...current.gallery, images: current.gallery.images.filter((item) => item.id !== image.id) } }))} className="mt-1 text-[11px] font-bold text-error">Quitar</button></div>)}</div>{config.gallery.images.length < 12 ? <label className="relative inline-flex cursor-pointer rounded-lg border border-outline-variant/30 px-3 py-2 text-xs font-bold text-on-surface"><input type="file" multiple accept="image/jpeg,image/png,image/webp,image/avif" className="sr-only" disabled={uploading} onChange={async (event) => { const files = Array.from(event.target.files ?? []).slice(0, 12 - config.gallery.images.length); const uploaded: SiteImage[] = []; for (const file of files) { const url = await uploadImage(file); if (url) uploaded.push({ id: crypto.randomUUID(), url, alt: "" }); } if (uploaded.length) onChange((current) => ({ ...current, gallery: { ...current.gallery, images: [...current.gallery.images, ...uploaded].slice(0, 12) } })); event.target.value = ""; }} />{uploading ? "Subiendo…" : "Agregar imágenes"}</label> : null}</div>; }
