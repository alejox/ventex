...
export function CardMethodSelector({
  enabledMethods,
  selectedMethod,
  onSelect,
}: CardMethodSelectorProps) {
  // Filtrar solo los medios de tarjeta habilitados en la configuración del negocio
  const methodsToDisplay = COLOMBIA_CARD_METHODS.filter((m) =>
  ...
  if (methodsToDisplay.length === 0) {
    return (
      <div className="p-3 text-xs text-on-surface-variant bg-surface-container rounded-xl border border-outline-variant/20 text-center">
        No hay medios de tarjeta habilitados en Configuración.
      </div>
    );
  }
