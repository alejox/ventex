...
### Routing & auth flow
- `app/(auth)/` — route group for `login`, `register`, `reset-password`, `update-password` (shared auth layout). `login/page.tsx` has two modes: **Dueño** (owner — email+password or Google) and **Empleado** (worker — business key + username; see Roles & tenancy below).
