...
Required env vars: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, and `NEXT_PUBLIC_SITE_URL` (used for the signup email redirect **and** for the response/confirmation URLs handed to ePayco; falls back to `http://localhost:3000` — a hostname ePayco refuses, so `publicUrlBase()` rewrites it to `127.0.0.1`). `NEXT_PUBLIC_API_URL` is optional (base URL for the axios HTTP client — see data flow below). **`.env.example` is the full list** — including the four ePayco keys and `CRON_SECRET` described in AGENTS.md's *Subscription billing* section.
