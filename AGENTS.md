...
- **`apply_billing_charge(order, user)`**: `user` may be **null** (guest checkout) — marks the order paid without activating a licence; `claim_guest_orders(email)` activates it when the visitor registers with that same email. A `client_licenses` row always wins over online billing (the reseller recharges it), so recurrence is forced off for those users.
